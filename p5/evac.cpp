...
void Evac::BFS (int * evacIDs)
{
	...
	for (int i = 0; i < numEvacCities; i++)
	{
		for (int j = 0; j < allCities[evacIDs[i]].roadCount; j++)
		{
			if (!bfsVisited[allCities[evacIDs[i]].roads[j].ID])
			{
				...
				if (allCities[evacIDs[i]].roads[j].ID % 2 == 0)
					bfsVisited[allCities[evacIDs[i]].roads[j].ID + 1] = true;
				else
					bfsVisited[allCities[evacIDs[i]].roads[j].ID - 1] = true;
			}
		}
