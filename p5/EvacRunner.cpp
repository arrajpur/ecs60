  ...
  int routeCount, Road2 *roads)
{
  ...
  {
//    cout << "Route checker id is " << evacRoutes[i].roadID << endl;
  ...
  for(multiset<EvacRoute>::iterator itr = routes.begin(); itr != routes.end(); itr++)
  {
    ...
    if(itr->numPeople > 0)
    {
	  ...
	  << " with peoplePerHour " << roads[itr->roadID].peoplePerHour << endl;
        return;
      }
...
//  cout << "right after cpu time" << endl;

  ifstream inf2(argv[1]);

//  cout << "after ifstream" << cout;

  inf2 >> numCities >> numRoads >> numEvacs;

//  cout << "before allocs" << endl;
